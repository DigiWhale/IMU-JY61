...
def open_serial_connection_and_print_output():
    ...
    while True:
        ...
        # print(data)
        header = binascii.hexlify(data[0:1]).decode('ascii')
        sensor = binascii.hexlify(data[1:2]).decode('ascii')
        reg_1 = binascii.hexlify(data[2:4]).decode('ascii')
        reg_2 = binascii.hexlify(data[4:6]).decode('ascii')
