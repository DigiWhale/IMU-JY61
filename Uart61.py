...
def open_serial_connection_and_print_output():
    ...
    while True:
        ...
        # print(data)
        header = int(binascii.hexlify(data[0:1]).decode('ascii'))
        sensor = int(binascii.hexlify(data[1:2]).decode('ascii'))
        reg_1 = int(binascii.hexlify(data[2:4]).decode('ascii'))
        reg_2 = int(binascii.hexlify(data[4:6]).decode('ascii'))
        reg_3 = int(binascii.hexlify(data[6:8]).decode('ascii'))
        reg_4 = int(binascii.hexlify(data[8:10]).decode('ascii'))
        checksum = int(binascii.hexlify(data[10:11]).decode('ascii'))
